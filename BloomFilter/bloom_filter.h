...
class BloomFilter {
  static_assert(
    L <= 64, "specified L is too long for a counting Bloom filter! Maybe 4 is enough! ");
  ...
  inline uint64_t getAt(uint32_t index) const {
    uint32_t start = (uint64_t) index * L / 64;
...
public:
  ...
  explicit BloomFilter(uint32_t capacity = 4096, float FP = 0.001, int8_t numberOfHashes = -1)
    : capacity(capacity) {
    if (numberOfHashes == -1) {
      int tmpM = m = capacity;
      int tmpK = k = 4;
...
//    cout << "decided m, K, fp: " << m << ", " << K << ", " << fp << endl;
    ...
    for (int i = 0; i < k; ++i) {
      h.push_back(Hasher32<Key>(uint32_t(rand())));
    }
  }
  ...
  //! return the bit pattern to erase: if the i-th is 1, then remove the i-th mapped bit for the Bloom filter
